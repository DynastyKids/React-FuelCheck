...
function contentText(getBounds, data, userfuel) {
    ...
    const fuelName = [
        ["U91", "E10", "P95", "P98", "DL", "PDL", "B20", "LPG", "E91", "P9X", "DLS", "All"],
        ["Unleaded 91", "Ethanol 10", "Premium Unleaded 95", "Premium Unleaded 98", "Diesel", "Premium Diesel", "BioDiesel", "LPG", "Unleaded 91 & E10", "Premium 95 & 98", "Diesel & Premium Diesel", "All"]
    ]
    const { _northEast, _southWest } = getBounds;

    let index = fuelName[0].indexOf(userfuel);
        ...
        for (let dataindex = 0; dataindex < data.data.length; dataindex++) {
            ...
            data.data[dataindex].forEach(element => {
                if (element.loc_lat < _northEast.lat && element.loc_lat > _southWest.lat && element.loc_lng > _southWest.lng && element.loc_lng < _northEast.lng) { // Check element geolocation in range
                    ...
                    } else if (userfuel === "E91") { // User is selecting U91+E10
                        if (element.E10 && element.E10 !== null && cheapestElement[fuelName[0][index]] > element.E10) {
                            cheapestElement = element
                            cheapestElement[fuelName[0][index]] = element.E10
                        }
                        if (element.U91 && element.U91 !== null && cheapestElement[fuelName[0][index]] > element.U91) {
                            cheapestElement = element
                            cheapestElement[fuelName[0][index]] = element.U91
                        }
...
const Location = ({ map, data, status, userfuel }) => {
    ...
    useEffect(() => {
        ...
        map.on('moveend zoomend', () => {
            ...
            info.textContent = contentText(bounds, data, userfuel);
