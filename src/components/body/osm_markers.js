...
export default function OsmMarkers(props) {
    ...
    const filterDatas = () => {
        ...
        if (window.location.hash.length > 1) {
            ...
            props.jsondata.forEach(element => {
                ...
                newdata.push({ "address": element.address, "brand": element.brand, "loc_lat": element.loc_lat, "loc_lng": element.loc_lng, "name": element.name, "priceInfo": priceInfo })
            });
        }
    ...
    return (
        <>
            ...
            {filteredData.map(({ brand, name, loc_lat, loc_lng, address, priceInfo, price1 }, index) => (
